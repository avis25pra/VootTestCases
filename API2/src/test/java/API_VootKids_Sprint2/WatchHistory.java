...
public class WatchHistory extends GenericMethod_VK
{
		 ...
		 for(int i=1; i<=rowCount;i++)
	        {
		    	//reading values from excel
			 	...
			 	{
			 		WatchHistory.NotPasslimit(i, URL);//calling function when limit is not passed
			 		continue;
			 	}
			 	if(offSet.equals("EMPTY"))//when limit is empty initialize it to ""
			 	{
			 		offSet="";
			 	}
			 	if(offSet.equals("NOTPASS"))
			 	{
			 		WatchHistory.NotPassoffset(i, URL);//calling function when offset is not passed
			 		continue;
			 	}
			 	if(ks.equals("EMPTY"))//when ks is empty initialize it to ""
			 	{
			 		ks="";
			 	}
			 	if(ks.equals("NOTPASS"))
			 	{
			 		WatchHistory.NotPassks(i, URL);//calling function when ks is not passed
			 		continue;
			 	}
	           ...
	           //writing back to excel
	             GenericMethod_VK.ExcelWriteBack(path2, "Watch_history", i, 9, resp1, 10, TestType, counter, bool, myValue, Value2test, 11, 12,mynullKeys,myErrorArray );
		}
//		 GenericMethod_VK.write2Master(1, "Watch_history", 10,path2);
