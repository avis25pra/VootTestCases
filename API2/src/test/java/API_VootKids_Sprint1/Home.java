...
public class Home extends GenericMethod_VK 
{
	...
	public void home() throws EncryptedDocumentException, InvalidFormatException, IOException
	{
        ...
        {
	    	Row row=sh.getRow(i);
	    	offSet=row.getCell(3).getStringCellValue();
	    	limit=row.getCell(2).getStringCellValue();
	    	...
	    	if(TestType.equals("Positive") && i<=5)//logic for positive scenarios
			{
	    	...
	    	if(TestType.equals("Positive") && i>5)
	    	{
	    		//declaration of Arraylist for null parameters keys
				ArrayList<String> nullParams = new ArrayList<String>();//storing the keys which have null values
				ArrayList<String> myDatatype = new ArrayList<String>();//storing the datatype of keys coming from response
			...
			GenericMethod_VK.ExcelWriteBack(path1, "Home", i, 8, resp1, 9, TestType, counter, bool, myValue, Value2test, 10, 11,mynullKeys,myErrorArray );
	    	
        }
	    GenericMethod_VK.write2Master(13,"Home",9,path1);
