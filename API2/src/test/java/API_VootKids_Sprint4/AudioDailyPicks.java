...
public class AudioDailyPicks extends GenericMethod_VK
{
	...
	@Test
	public void audioDailyPicks() throws EncryptedDocumentException, InvalidFormatException, IOException 
	{
		RestAssured.config = RestAssured.config().encoderConfig(EncoderConfig.encoderConfig().appendDefaultContentCharsetToContentTypeIfUndefined(false));
		//reading data from excel
		FileInputStream fis=new FileInputStream(path4);
		Workbook wb=WorkbookFactory.create(fis);
		Sheet sh=wb.getSheet("AudioDailyPicks");
	    ...
	    for(int i=1; i<=rowCount;i++)
        {
	    	...
	    	{
	    		AudioDailyPicks.notPassLimit(i, Url);
	    		continue;
	    	}
	    	...
	    	{
	    		AudioDailyPicks.notPassOffset(i,Url);
	    		continue;
	    	}
	    	if(limit.equals("NA") && offSet.equals("NA"))
	    	{
	    		AudioDailyPicks.nonmandatoryparameters( i, Url);
	    		continue;
	    	}
			...
			//writing back to excel
             GenericMethod_VK.ExcelWriteBack(path4, "AudioDailyPicks", i, 8, resp1, 9, TestType, counter, bool, myValue, Value2test, 10, 11,mynullKeys,myErrorArray );
        }
	 	GenericMethod_VK.write2Master(11, "AudioDailyPicks", 9,path4);
