...
import API_VootKids_Sprint3.AllEpisode;

public class GetEbookList extends GenericMethod_VK
{
	...
	@Test
	public void getEbookList() throws EncryptedDocumentException, InvalidFormatException, IOException 
	{
		...
		Sheet sh=wb.getSheet("GetEbookList");
		...
		for(int i=1; i<=rowCount;i++)
		{
            ...
            {
            	GetEbookList.NotPassFilterBy(i, URL);//calling function when limit is not passed
            	continue;
            }
            ...
            {
            	GetEbookList.NotPassFilterValue(i, URL);//calling function when limit is not passed
            	continue;
            }
            ...
            {
            	GetEbookList.NotPasslimit(i, URL);//calling function when limit is not passed
            	continue;
            }
         	...
         	{
         		GetEbookList.NotPassoffSet(i, URL);//calling function when offset is not passed 
         		continue;
         	}
			...
			//writing back to excel
             GenericMethod_VK.ExcelWriteBack(path4, "GetEbookList", i, 10, resp1, 11, TestType, counter, bool, myValue, Value2test, 12, 13,mynullKeys,myErrorArray );
				
        }
	 	GenericMethod_VK.write2Master(6, "GetEbookList", 11,path4);
